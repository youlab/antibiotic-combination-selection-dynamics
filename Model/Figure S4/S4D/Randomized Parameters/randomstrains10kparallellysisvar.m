...
data = nan(nstrains, 14); % for 12 parameters + sensitive, resistant cell densities
%%
tic % start timer
parfor k = 1:nstrains   % for each strain
    ...
    intercept = 0.3 * rand(); % basal lysis rate 
    ...
    for i = 1:length(antibiotics)   % for each antibiotic concentration
        ...
        for j = 1:length(inhibitors)   % for each inhibitor concentration
            ...
            p= [xi;kappab;phimax;da;ha;gamma;db;hi;alpha;betamin;c;thisinh; intercept];   % parameter values (initial conditions)
            options = odeset('RelTol',1e-10,'AbsTol',1e-10, 'NonNegative', [1:2]);   % set acceptable error
            [time, y] = ode45(@nondimODEsvariable2, time, y0, options, p);   % solve differential equations with specified inputs
    ...
    if (minA~=0 && minInh~=0)
        tempdata(13) = nsatFinalT(minA, minInh);   % sensitive cell density for the lowest resistant fraction along the isobole
        tempdata(14) = nratFinalT(minA, minInh);   % resistant cell density for the lowest resistant fraction along the isobole
    end
...
save("randomstrains_10k_lysisvar.mat", "data");
