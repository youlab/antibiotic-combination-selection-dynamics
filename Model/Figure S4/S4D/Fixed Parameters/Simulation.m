...
hi=2;
gamma=1.38;  
intercept = 0.15; % up to 0.3 in Kim et al.
    ...
    for j = 1:length(inhibitors)
        ...
        for k = 1:length(betamins)
            ...
            for m = 1:length(cs)
                ...
                p= [xi;kappab;phimax;da;ha;gamma;thisbetamin;alpha;db;hi;thisinh;thisc;intercept];
                options = odeset('RelTol',1e-10,'AbsTol',1e-10); % reduce step size, reduce artifacts
                % solve ODEs
                [time, y] = ode45(@nondimODEs4, time, y0, options, p); 
                % save variable data
                nr(i, j, k, m, :) = y(:,1); 
                ns(i, j, k, m, :) = y(:,2); 
                b(i, j, k, m, :) = y(:,3);
...
for i = 1:4
    handles(i) = plot(inhibitors, squeeze(fraction_subthreshold(ibetamin, ic, :)), 'Color', colors(i+1, :), 'LineWidth', 2);
end
