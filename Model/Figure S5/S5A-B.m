...
s0 = 10;
b0 = 0;    
antibiotics = logspace(log10(1), log10(100), 15); 
inhibitors = logspace(log10(1), log10(100), 15);
...
for i = 1:length(paramranges)
    paramvals(i, :) = linspace(paramranges(i, 1), paramranges(i, 2), ntestpoints);
end
...
parameterid = 2;
pat = characterListPattern("\_{}");
% resistant fraction
for i = 1:ntestpoints
    fig = figure(23298370 + i);
    hold on
    imagesc([min(inhibitors) max(inhibitors)], [max(antibiotics) min(antibiotics)], flipud(squeeze(ODatFinalT(parameterid, i, :, :))))
    %colorbar
    clim([0 4.5])
    colormap(cmocean('tempo'))
    ax = gca;
    ax.YDir = 'normal';
    ax.XTick = linspace(min(inhibitors), max(inhibitors), (length(inhibitors)/7)+1);
    ax.YTick = linspace(min(antibiotics), max(antibiotics), (length(antibiotics)/7)+1);
    ax.XTickLabel = strtrim(cellstr(num2str(inhibitors(1:7:end)', '%2g'))');
    ax.YTickLabel = strtrim(cellstr(num2str(antibiotics(1:7:end)', '%2g'))');
    % xlabel("Inhibitor")
    % ylabel("Antibiotic")
    ...
    saveas(fig, strcat("density ", erase(paramnames(parameterid), pat), " ", replace(string(paramvals(parameterid, i)), ".", "'")), 'jpeg')
end

% resistant fraction
for i = 1:ntestpoints
    fig = figure(62378900 + i);
    hold on
    imagesc([min(inhibitors) max(inhibitors)], [max(antibiotics) min(antibiotics)], flipud(squeeze(fractionRatFinalT(parameterid, i, :, :))))
    set(gca, 'color', [0 0 0]);
    %colorbar
    clim([0 1])
    ...
    saveas(fig, strcat("resistance ", erase(paramnames(parameterid), pat), " ", replace(string(paramvals(parameterid, i)), ".", "'")), 'jpeg')
