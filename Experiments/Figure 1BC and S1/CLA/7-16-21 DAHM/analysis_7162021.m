...
strainname = 'DA28102 High Copy BlaM';

% read in raw data from excel
datafile = '7-16-2021 Data.xlsx';
...
layoutfile = '7-16-2021 Protocol.xlsx';
...
for hour = [4 8 16 24]
    ...
    figure(200+hour)
    imagesc((finalgfp ./ finalOD))
    colorbar
    caxis([0 3000])
    colormap((cmocean('curl')))
    ax = gca;
    ax.YDir = 'normal';
    %ax.XTick = linspace(min(inhibitors), max(inhibitors), (length(inhibitors)/5+1));
    %ax.YTick = linspace(min(antibiotics), max(antibiotics), (length(antibiotics)/5+1));
    %ax.XTickLabel = strtrim(cellstr(num2str(inhibitors(1:5:end)', '%.1f'))');
    %ax.YTickLabel = strtrim(cellstr(num2str(antibiotics(1:5:end)', '%.1f'))'); 
    ax.XTickLabel = strtrim(cellstr(num2str(inhibitors(2:2:end)'))');
    ax.YTickLabel = strtrim(cellstr(num2str(antibiotics'))'); 
    xlabel("clavulanic acid (\mug/mL)")
    ylabel("Amoxicillin (\mug/mL)")
    title(strainname + " " + hour +"h GFP/OD")
    set(gca, 'fontsize', 20)
    axis square
    hold off
end
